...
def convert_to_soup(data) -> bs:
    """
    Converts data to BeautifulSoup file.
    """
...
def find_name(soup: bs) -> str:
    ...
    return code_filter[2].string[7:].strip()
...
def filter_table_1(data: list) -> list[str]:
    """
...
def main(list_of_references: list[str]) -> list[list[str]]:
    """
    Main function filters data from columns:
    village code number, name, voters on the list,
    issued envelopes and valid votes from tables under url.
    ...
    for e, reference in enumerate(list_of_references):
        ...
        village_soup = convert_to_soup(village_html)
        ...
        table_1_result = filter_table_1(table_1_columns[0])
...
references_soup = convert_to_soup(references_html)
